# %%
"""
This .py provides an individual evaluation of the block CCO2, which is a current-controlled
oscillator with a comparator that generates the coarse conversion of the pipeline converter.
"""
import random
...
# --- Noise Generator Function for the Ioffset Current Source ---
def generar_ruido_Ioffset(n_filas, n_columnas, fs, psd_white, psd_f1Hz, gbw_ota):
    nyquist = 0.5 * fs
    ...
    # 1. Define the geometric profile of the 1/f filter in power.
    # Since power decays as (1/f), the spectrum amplitude decays as 1/sqrt(f).
    with np.errstate(divide='ignore', invalid='ignore'):
        filtro_flicker = 1.0 / np.sqrt(freqs)
    # Remove the DC component (0 Hz) to avoid division by zero (infinity).
    filtro_flicker[0] = 0

    # 2. RADICAL GENERALIZATION: Find the first physical measurable frequency.
    # It does not matter if it is 100 Hz, 1 Hz, or 0.1 Hz. It is the first useful bin after DC.
    f_primera_medible = freqs[1]

    # Apply the physical law of real flicker noise (PSD = psd_f1Hz / f).
    # Calculate exactly how much power (A²/Hz) the circuit MUST have at that first frequency.
    psd_objetivo_primera_freq = psd_f1Hz / f_primera_medible

    # Evaluate the power generated by the mathematical base profile at that same point (bin [1]).
    # Squaring the filter amplitude provides its intrinsic unscaled power.
    ...
    # Square root is applied because this factor multiplies the noise AMPLITUDE.
    ...
    for i in range(n_filas):
        ...
        # Scale the time vector to match real physical current magnitudes (A)
        flicker[i, :] = flicker_temporal_puro * factor_escala

    # =========================================================================
    # --- STEP 3: COHERENT SUMMATION AND FINAL OTA FILTERING ---
    # =========================================================================

    # Combine both two-dimensional matrices (Thermal Noise + Flicker Noise)
    ruido_total_raw = thermal_scaled + flicker

    # Limit the total noise bandwidth using the low-pass filter modeling the OTA
    b_ota, a_ota = butter(1, gbw_ota / nyquist, btype='low')
    ruido_filtrado = lfilter(b_ota, a_ota, ruido_total_raw, axis=1)

    return ruido_filtrado


# --- Application of the Noise Generator ---
# Schematic value: (1 / (2 * pi * CL * Rc)) Hz
gbw_ota = gm_ota / (2 * np.pi * Cl_ota)
psd_white_cadence = 1e-27  # A^2/Hz, Cadence simulation value
# A^2/Hz at 1 Hz, Cadence simulation value
psd_flicker_1Hz_cadence = 2.9e-27

ruido_Ioffset = generar_ruido_Ioffset(
    n_filas=Iin.shape[0],
    n_columnas=time_x,
    fs=Fs,
    psd_white=psd_white_cadence,
    psd_f1Hz=psd_flicker_1Hz_cadence,
    gbw_ota=gbw_ota
)

# Create the base current matrix (expanded Iin)
Ipd_base = np.tile(Iin[:, np.newaxis], (1, time_x))
# Combine current matrices
Ipd_matrix = Ipd_base + Ioffset + (ruido_Ioffset * enable_noise_Ioffset)
...
Vcoarse_preamp_matrix, counter_preamp_matrix = Icco_preamp.integrar_fila(
    Ipd_matrix, VCPnoisy, Vref_comparator_noisy, ota_noise, Thermal_noise_preamp)
...
coefs_frec = np.polyfit(Iin_mean, Frec_mean, 2)
K_cco = coefs_frec[0]  # in Hz/A
Frec_i_fit = np.polyval(coefs_frec, Iin_mean)
INL_i_Frec = ((Frec_mean - Frec_i_fit) / np.max(Frec_mean)) * 100  # in %
INL_ppm_FSR = ((Frec_mean - Frec_i_fit) / np.max(Frec_mean)) * 1e6
INL_ppm_reading = ((Frec_mean - Frec_i_fit) / Frec_mean) * 1e6
print(f"Polynomial fit coefficients: {coefs_frec}")
print(
    f"Maximum INL in ppm relative to FSR: {np.max(np.abs(INL_ppm_FSR)):.2f} ppm")
print(
    f"INL ppm FSR for the first input current: {np.abs(INL_ppm_FSR[0]):.2f} ppm")
print(
    f"Maximum INL in ppm relative to reading: {np.max(np.abs(INL_ppm_reading)):.2f} ppm")
print(
    f"INL ppm reading for the first input current: {np.abs(INL_ppm_reading[0]):.2f} ppm")
